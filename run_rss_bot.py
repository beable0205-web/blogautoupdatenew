...
- 본문 중간과 결론부에 call to action 문구(예: "이벤트 혜택이 언제 종료될지 모르니 일단 확인부터 해보세요!")를 적어줄 것."""
    ...
    # 1-Pass 통합 프롬프트 작성 (AI 인용 친화형 고구조 콘텐츠 작성 가이드 완벽 융합 + 홈판 벤치마킹 스타일 가이드)
    prompt = f"""
    ...
    * 입력 처리 규칙에 따른 판별 가이드:
    ...
    3. 진정성과 신뢰성: 확인되지 않은 뇌피셜 정보는 단정하지 마십시오. 과장이나 낚시성 표현을 피합니다. 건강 정보인 경우 반드시 유효 약리 성분(알리신, 라이코펜, 설포라판 등)의 명칭과 과학적 원리를 쉽고 분명히 명시하십시오.
    ...
    5. **1인칭 주관적 의견 및 통찰 15% 비중 의무화:**
       - 단순 팩트 설명 나열에서 벗어나, "제 생각은 조금 다릅니다", "제가 직접 살펴보니 이건 이렇더군요", "왜 무주택자만 힘들어지는 구조인지 그 이면의 진짜 이유는..." 처럼 **자신의 뚜렷한 철학, 비평 및 주관적 생각**을 반드시 15% 이상 분량으로 서술해 주십시오.
    ...
    {blog_insight_guide}
    [HTML 레이아웃 및 출력 양식 규칙 (네이버 모바일 홈판 벤치마킹 스타일)]
    1. 100% HTML 태그로 구조화 작성 (마크다운 기호 `*`, `-`, `##` 등 절대 금지). 모든 인라인 스타일 속성은 홑따옴표(')를 사용하십시오.
    2. **모바일 가독성 특화 단문(Short-line) 줄바꿈 강제:**
       - 스마트폰 화면 가로너비에서 줄바꿈이 지저분하게 깨지는 현상을 방지하고 리듬감을 높이기 위해, 한 줄당 평균 15~20자 내외로 호흡을 쪼개어 문장 중간에 의도적으로 자주 줄바꿈(HTML `<br>`) 처리를 하십시오.
       - 예시:
         몸이 가볍게 움직일 때는<br>
    ...
    5. **[필수] 본문 content의 HTML 구조 규격 (6단계 구조식)**:
       ...
       - **(3) 본문**: 소제목 3~6개로 구분. 각 소제목은 `<h2>` 혹은 `<h3>`에 고품질 언더라인/테두리 인라인 스타일을 주거나, 소제목 중 1~2개는 아래의 **인용구(Blockquote) 소제목 마크업**을 사용하여 래핑하십시오:
