...
- 본문 중간과 결론부에 call to action 문구(예: "이벤트 혜택이 언제 종료될지 모르니 일단 확인부터 해보세요!")를 적어줄 것."""
    ...
    [공통 필수 준수 가이드]
    1. 분량: 공백 제외 800자 ~ 1,000자. 모바일 가독성 극대화.
    ...
    4. 100% HTML 태그로 구조화 작성 (마크다운 기호 `*`, `-`, `##` 등 절대 금지):
       - 말투는 반드시 "~음", "~였음", "~인 듯함", "~라고 보면됨", "~은 아님" 등의 츤데레 1인칭 독백체로 고정하고, 존댓말("~요", "~입니다")은 100% 철저히 배제할 것.
       - 모바일 뷰어 최적화 가독성을 위해 **글의 띄어쓰기를 극대화**할 것: 
         * 문장은 길게 늘어뜨리지 말고 무조건 단 1~2개 단위로 쪼개어 개별 `<p style='font-size: 16px; line-height: 1.9; margin-bottom: 35px; color: #333; letter-spacing: -0.5px;'>...</p>` 문단으로 감쌀 것.
         * 문장과 문단 사이에 넉넉한 띄어쓰기를 보장하기 위해 마진 값(`margin-bottom: 35px` 이상)을 활용하고 필요시 빈 문단(`<p>&nbsp;</p>`)을 단락 중간중간에 삽입하여 시원시원한 띄어쓰기 가독성을 고수할 것.
    ...
    6. 면책 조항(Disclaimer)의 100% 무작위 동적 생성: 
       - 글의 맨 마지막(결론 및 해시태그 바로 위)에 🚨 [팩트체크 및 면책고지] 문구를 매번 완전히 다른 독백 어투로 지어내어 `<p style='font-size: 13px; color: #888; text-align: center; line-height: 1.6;'><b>🚨 [팩트체크 및 면책고지]</b><br>...</p>` 형식으로 생성.
    7. 썸네일 카피라이팅 후킹 지침:
