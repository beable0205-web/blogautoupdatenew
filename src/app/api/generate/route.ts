...
이 필터를 완벽히 부수기 위해 아래의 **초사실주의 1인칭 날것 스토리텔링** 및 **동적 구조 파괴(Dynamic Break) 규칙**을 반드시 적용하세요.
...
- 팩트가 확인되지 않은 가상의 액수나 일정은 절대 지어내지 마세요.
...
* **스타일 A [스토리텔링 및 리스크 밀착형]**:
  - 전개 순서: [도입부 극사실주의 가상 스토리] -> [경험을 통해 알아낸 진짜 꿀팁과 주의사항(주의할 리스크) 먼저 방출] -> [본문 상세 정보 분석 및 핵심 조건] -> [📌 한눈에 보는 핵심 팩트체크 요약 리스트] -> [친근한 유대감 1줄 요약 결론]
* **스타일 B [속전속결 팩트 폭격형]**:
  - 전개 순서: [도입부 가벼운 일상 공감] -> [💡 바쁘신 분들을 위한 오늘의 정책 팩트체크 리스트 전면 배치] -> [본문 조건 및 혜택 상세 세부 분석] -> [🔑 놓치면 안 되는 상위 1% 딥 인사이트 주의사항] -> [최종 결론 요약 및 다정한 아웃트로]
* **스타일 C [장단점 입체 비교형]**:
  - 전개 순서: [도입부 현업의 상황/문제 직시] -> [본문 조건/상세 분석] -> [유사 상품/타 정책과의 구체적인 수치적 비교 및 장단점 분석] -> [김쌤이 알려주는 가장 실속 있는 활용 전략] -> [☘️ 최종 결론 핵심 요점 요약]

[단락 가독성 극대화 및 다양성 규칙]
1. 각 단락의 소제목(대주제/소주제)은 절대 "1. 핵심 조건 분석" 같이 정적인 표현을 지양하고, 매번 주제에 맞추어 역동적인 이웃의 목소리로 가공하세요.
   - 예: "📌 신청 전 무조건 보셔야 할 3초 팩트체크", "💡 제가 직접 살펴보고 깜짝 놀란 주의사항 2가지"
2. 표(\`<table>\`) 대신 스마트폰에서 깨지지 않도록 이모지(✔️, 📌, 💡, 🔑)를 활용해 깔끔한 줄바꿈 리스트 형태로 서술하세요.
3. 복잡한 퍼센트보다 [구체적인 액수/수익 차이]를 직접 숫자로 명시해서 체감도를 높이세요. (예: "똑같이 3천만원을 1년 굴려도 A기관보다 B기관에서 이자 472,380원을 더 받습니다!")
4. 긴 글은 스마트폰 가독성을 위해 반드시 2~3줄 단위로 짧게 쪼개어 단락 간 여백을 풍부하게 살리세요.
`;
    }
