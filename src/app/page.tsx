  ...
  const handleGenerate = async (e?: React.FormEvent | null, overrideKeyword?: string, category: string = 'general') => {
    ...
    try {
      ...
      if (mode === 'keyword') {
        ...
        // PDF 파일을 바이너리 스트림 형식으로 직접 본문에 담아 송신 (Next.js multipart/JSON 413 용량 초과 제한 우회)
        response = await fetch(`/api/generate-pdf?deviceType=mobile&category=${category}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/pdf',
          },
          body: pdfFile!, // raw binary stream
        });
      }
